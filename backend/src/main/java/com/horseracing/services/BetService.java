...
public class BetService {
    ...
    public BetResponse placeBet(User user, PlaceBetRequest request) {
        ...
        if (role != Role.SPECTATOR && role != Role.HORSE_OWNER && role != Role.JOCKEY) {
            throw new BusinessException(
                    "Only Spectators, Horse Owners, and Jockeys are allowed to place bets.",
        ...
        if (role == Role.JOCKEY) {
            boolean isParticipating = raceParticipantRepository.existsByJockeyUserIdAndTournamentId(
                    user.getId(), race.getTournament().getId());
            if (isParticipating) {
                throw new BusinessException(
                        "Jockeys are not allowed to place bets in tournaments they participate in.",
                        HttpStatus.FORBIDDEN);
            }
        } else if (role == Role.HORSE_OWNER) {
            boolean isParticipating =
                    raceParticipantRepository.existsByHorseOwnerUserIdAndTournamentId(user.getId(),
        ...
        // Betting is allowed when race is open for register, closed for register, or locked list
        // (before running)
        ...
        if (!isBettingAllowedStatus) {
            throw new BusinessException("Betting is currently closed for this race.",
        ...
        if (isSimFinished) {
            throw new BusinessException("The race has finished and betting is closed.",
                    HttpStatus.BAD_REQUEST);
        }
        ...
        if ("DISQUALIFIED".equalsIgnoreCase(participant.getStatus())) {
            throw new BusinessException("This horse has been disqualified before the race.",
                    HttpStatus.BAD_REQUEST);
        }
        ...
        if (minBet != null && request.getAmount().compareTo(minBet) < 0) {
            throw new BusinessException("Minimum bet amount is " + minBet + " VND.",
                    HttpStatus.BAD_REQUEST);
        }
        ...
        Wallet wallet = walletRepository.findByUserIdWithLock(user.getId())
                .orElseThrow(() -> new BusinessException(
                        "User wallet not found. Please contact Administrator.",
                        HttpStatus.BAD_REQUEST));

        if (wallet.getBalance().compareTo(request.getAmount()) < 0) {
            throw new BusinessException("Insufficient wallet balance to place bet.",
                    HttpStatus.BAD_REQUEST);
        }
        ...
        // 7. Create Bet (Odds is initially 1.0 in Pari-Mutuel and will be calculated upon
        // completion)
        Bet bet = Bet.builder().user(user).race(race).participant(participant)
                .amount(request.getAmount()).odds(BigDecimal.ONE).status("PENDING")
                .betType(request.getBetType().toUpperCase()).build();
        bet = betRepository.save(bet);

        // 8. Create WalletTransaction
        WalletTransaction walletTx = WalletTransaction.builder().wallet(wallet)
                .transactionType("BET").amount(request.getAmount()).status("SUCCESS")
        ...
        BettingTransaction bettingTx =
                BettingTransaction.builder().bet(bet).walletTransaction(walletTx).build();
    ...
    public List<BetResponse> getUserBets(User user) {
        ...
        return bets.stream().map(BetResponse::fromEntity).collect(Collectors.toList());
    }
}
