...
public class FeedbackServiceImpl implements FeedbackService {
    ...
    public FeedbackResponse createFeedback(String identifier, CreateFeedbackRequest request) {
        ...
        Feedback feedback = Feedback.builder().user(user).subject(request.getSubject().trim())
                .content(request.getContent().trim()).status("PENDING").build();
    ...
    public List<FeedbackResponse> getAllFeedbacks(String status, String role, String search) {
        ...
        return list.stream().filter(fb -> {
            ...
            if (search != null && !search.isBlank()) {
                ...
                boolean matchName = fb.getUser() != null && fb.getUser().getFullName() != null
                        && fb.getUser().getFullName().toLowerCase().contains(term);
                boolean matchEmail = fb.getUser() != null && fb.getUser().getEmail() != null
                        && fb.getUser().getEmail().toLowerCase().contains(term);
                boolean matchSubject =
                        fb.getSubject() != null && fb.getSubject().toLowerCase().contains(term);
                boolean matchContent =
                        fb.getContent() != null && fb.getContent().toLowerCase().contains(term);
            ...
            return true;
        }).map(FeedbackResponse::fromEntity).collect(Collectors.toList());
    }
    ...
    public FeedbackResponse resolveFeedback(Integer feedbackId, ResolveFeedbackRequest request) {
        ...
        try {
            if (updated.getUser() != null) {
                notificationService.sendNotification(updated.getUser(),
                        "Feedback Response #" + feedbackId,
                        "Your feedback (\"" + updated.getSubject()
                                + "\") has been processed by Administration: "
                                + updated.getAdminNote(),
    ...
    public FeedbackResponse rejectFeedback(Integer feedbackId, ResolveFeedbackRequest request) {
        ...
        try {
            if (updated.getUser() != null) {
                notificationService
                        .sendNotification(updated.getUser(), "Feedback Response #" + feedbackId,
                                "Your feedback (\"" + updated.getSubject()
                                        + "\") was rejected. Reason: " + updated.getAdminNote(),
                                NotificationType.GENERAL);
            }
        } catch (Exception e) {
