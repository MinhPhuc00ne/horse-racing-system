...
public class PublicApiService {
    ...
    public PublicLeaderboardResponse getLeaderboard() {
        List<JockeyProfile> jockeys = jockeyProfileRepository
                .findAllByOrderByRankingScoreDescWinRateDesc(PageRequest.of(0, 5));
        ...
        for (JockeyProfile j : jockeys) {
            String name = (j.getUser() != null && j.getUser().getFullName() != null)
                    ? j.getUser().getFullName()
                    : "Unknown Jockey";
            jockeyDtos.add(JockeyLeaderboardDto.builder().rank(jRank++).fullName(name)
                    .winRate(j.getWinRate() != null ? j.getWinRate() : 0.0)
                    .rankingScore(j.getRankingScore() != null ? j.getRankingScore() : 0).build());
        }

        List<HorseLeaderboardDto> horseDtos = new ArrayList<>();
        int hRank = 1;
        for (Horse h : horses) {
            String breedName = (h.getBreed() != null && h.getBreed().getBreedName() != null)
                    ? h.getBreed().getBreedName()
                    : "Unknown Breed";
            horseDtos.add(HorseLeaderboardDto.builder().rank(hRank++).horseName(h.getName())
                    .breedName(breedName)
                    .rating(h.getSpeedRating() != null ? h.getSpeedRating() : 50.0).build());
        }

        return PublicLeaderboardResponse.builder().jockeys(jockeyDtos).horses(horseDtos).build();
    }
    ...
    public PublicStatsResponse getStats() {
        ...
        return PublicStatsResponse.builder().activeTournaments(activeTournaments)
                .totalPrizePoolVND(totalPrizePoolVND != null ? totalPrizePoolVND : BigDecimal.ZERO)
                .activeHorses(activeHorses).build();
    }
}
