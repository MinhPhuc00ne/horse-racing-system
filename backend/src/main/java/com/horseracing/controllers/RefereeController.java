...
public class RefereeController {
    ...
    public ResponseEntity<List<RefereeRaceResponse>> getAssignedRaces(
        ...
        List<RefereeRaceResponse> response =
                refereeService.getAssignedRaces(userDetails.getUsername(), status);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/tournaments")
    public ResponseEntity<List<TournamentResponse>> getAssignedTournaments(
            Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
        List<TournamentResponse> response =
                refereeService.getAssignedTournaments(userDetails.getUsername());
        return ResponseEntity.ok(response);
    }

    @PutMapping("/tournaments/{tournamentId}/cancel-assignment")
    public ResponseEntity<MessageResponse> cancelAssignment(@PathVariable Integer tournamentId,
            Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
        refereeService.cancelAssignment(tournamentId, userDetails.getUsername());
        return ResponseEntity
                .ok(new MessageResponse("Successfully cancelled assignment for the tournament."));
    }

    @GetMapping("/inspect/horses")
    public ResponseEntity<List<Map<String, Object>>> getHorsesToInspect(
            Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
    ...
    public ResponseEntity<?> updateInspectionStatus(@PathVariable Integer participantId,
            @RequestBody Map<String, String> body, Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
        String status = body.get("status");
        String reason = body.getOrDefault("reason", "");
        refereeService.updateInspectionStatus(participantId, status, reason,
    ...
    public ResponseEntity<MessageResponse> updateJockeyWeight(@PathVariable Integer raceId,
            @PathVariable Integer jockeyId, @RequestBody Map<String, Double> body) {
        Double actualWeight = body.get("actualWeight");
        if (actualWeight == null) {
            return ResponseEntity.badRequest()
                    .body(new MessageResponse("actualWeight is required"));
        }
    ...
    public ResponseEntity<MessageResponse> disqualifyParticipant(@PathVariable Integer raceId,
            @PathVariable Integer participantId, @RequestBody Map<String, String> body) {
        ...
        return ResponseEntity.ok(new MessageResponse(
                "Participant has been disqualified. Betting orders for this horse have been refunded."));
    }
    ...
    public ResponseEntity<MessageResponse> updatePov(@PathVariable Integer raceId,
    ...
    public ResponseEntity<FlagResponse> addFlag(@PathVariable Integer raceId,
            @Valid @RequestBody AddFlagRequest request, Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
    ...
    public ResponseEntity<MessageResponse> addBlacklist(
            @Valid @RequestBody AddBlacklistRequest request, Authentication authentication) {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();
        refereeService.addBlacklist(request, userDetails.getUsername());
        return ResponseEntity.ok(
                new MessageResponse("Target has been added to blacklist and banned successfully."));
    }
    ...
    public ResponseEntity<MessageResponse> confirmResults(@PathVariable Integer raceId) {
        ...
        return ResponseEntity.ok(new MessageResponse(
                "Results confirmed. Prize distribution and bet payouts completed successfully."));
    }
