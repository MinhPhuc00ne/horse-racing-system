    ...
    public void run(String... args) throws Exception {
        ...
        if (raceTrackRepository.count() == 0) {
            List<RaceTrack> tracks = List.of(
                    ...
                    RaceTrack.builder().name("Jura Oval Track").location("Jura Great Forest").shape("OVAL").build()
            );
        ...
        try {
            ...
            for (int i = 0; i < spectators.length; i += 4) {
                if (!userRepository.existsByEmail(spectators[i + 1])) {
                    jdbcTemplate.update("INSERT INTO users (username, email, password, full_name, phone, provider, role, enabled) VALUES (?, ?, ?, ?, ?, 'LOCAL', 'SPECTATOR', 1)",
            ...
            String[] owners = {
                "benimaru", "benimaru@tempest.com", "Benimaru Commander", "0901000010", "Benimaru Stables", "East District, Tempest City",
                "souei", "souei@tempest.com", "Souei Shadow", "0901000011", "Souei Stables", "West District, Tempest City",
                "hakuro", "hakuro@tempest.com", "Hakuro Swordsman", "0901000012", "Hakuro Stables", "South District, Tempest City",
                "geld", "geld@tempest.com", "Geld Orc King", "0901000013", "Geld Stables", "North District, Tempest City",
                "gabil", "gabil@tempest.com", "Gabil Dragonewt", "0901000014", "Gabil Stables", "Dragon Shrine Lake District, Tempest",
                "rigurd", "rigurd@tempest.com", "Rigurd Goblin Prime", "0901000015", "Rigurd Stables", "City Center, Tempest",
                "gobta", "gobta@tempest.com", "Gobta Rider", "0901000016", "Gobta Stables", "Outskirts, Tempest City",
                "kaijin", "kaijin@tempest.com", "Kaijin Craftsman", "0901000017", "Kaijin Stables", "Craftsman District, Tempest"
            };
            for (int i = 0; i < owners.length; i += 6) {
                if (!userRepository.existsByEmail(owners[i + 1])) {
                    jdbcTemplate.update("INSERT INTO users (username, email, password, full_name, phone, provider, role, enabled) VALUES (?, ?, ?, ?, ?, 'LOCAL', 'HORSE_OWNER', 1)",
                    ...
                    jdbcTemplate.update("INSERT INTO horse_owner_profiles (user_id, stable_name, stable_address, phone, identity_number, date_of_birth, bank_account, description, reputation_stars, approval_status) VALUES (?, ?, ?, ?, ?, '1990-01-01', ?, 'Tempest Federation Ranch', 5.0, 'APPROVED')",
                            uId, owners[i + 4], owners[i + 5], owners[i + 3], "03809" + owners[i + 3], "9704" + owners[i + 3]);
                }
            }
            ...
            for (Object[] jck : jockeys) {
                ...
                if (!userRepository.existsByEmail(email)) {
                    jdbcTemplate.update("INSERT INTO users (username, email, password, full_name, phone, provider, role, enabled) VALUES (?, ?, ?, ?, ?, 'LOCAL', 'JOCKEY', 1)",
                    ...
                    jdbcTemplate.update("INSERT INTO jockey_profiles (user_id, height, weight, win_rate, experience_year, ranking_score, license_number, bank_account, approval_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'APPROVED')",
            ...
            for (int i = 0; i < referees.length; i += 4) {
                if (!userRepository.existsByEmail(referees[i + 1])) {
                    jdbcTemplate.update("INSERT INTO users (username, email, password, full_name, phone, provider, role, enabled) VALUES (?, ?, ?, ?, ?, 'LOCAL', 'RACE_REFEREE', 1)",
                            referees[i], referees[i + 1], commonPassword, referees[i + 2], referees[i + 3]);
                }
            }
            ...
            for (int i = 0; i < blacklisted.length; i += 5) {
                if (!userRepository.existsByEmail(blacklisted[i + 1])) {
                    ...
                    jdbcTemplate.update("INSERT INTO blacklist (target_type, target_id, reason, start_date, is_permanent, status, created_at) VALUES ('USER', ?, ?, GETDATE(), 1, 'ACTIVE', GETDATE())",
                            uId, blacklisted[i + 4]);
                }
            }
