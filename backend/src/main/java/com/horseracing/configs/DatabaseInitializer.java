...
public class DatabaseInitializer implements CommandLineRunner {
        ...
        try {
            ...
            if (count == null || count == 0) {
                jdbcTemplate.update("INSERT INTO roles (role_name, description) VALUES "
                        + "('ADMIN', 'System Administrator'), " + "('SPECTATOR', 'Normal User'), "
                        + "('HORSE_OWNER', 'Horse Owner'), " + "('JOCKEY', 'Jockey'), "
    ...
    private void ensureRaceTracks() {
        if (raceTrackRepository.count() == 0) {
            List<RaceTrack> tracks = List.of(
                    ...
                    RaceTrack.builder().name("Jura Circular Track").location("Jura Great Forest")
                            .shape("OVAL").build());
    ...
    private void ensureTensuraTestDataset() {
        ...
        try {
            ...
            String[] owners = {"benimaru", "benimaru@tempest.com", "Benimaru Commander",
                    "0901000010", "Benimaru Stables", "East District, Tempest City",
                    "Top racing stables in Tempest Federation", "souei", "souei@tempest.com",
                    "Souei Shadow", "0901000011", "Souei Stables", "West District, Tempest City",
                    "High-speed racing horse training experts", "hakuro", "hakuro@tempest.com",
                    "Hakuro Swordsman", "0901000012", "Hakuro Stables",
                    "South District, Tempest City",
                    "Prestigious ranch with legendary thoroughbreds", "geld", "geld@tempest.com",
                    "Geld Orc King", "0901000013", "Geld Stables", "North District, Tempest City",
                    "Endurance horse racing specialists", "gabil", "gabil@tempest.com",
                    "Gabil Dragonewt", "0901000014", "Gabil Stables",
                    "Dragon Shrine Lake District, Tempest", "Unique Dragonewt style racing stables",
                    "rigurd", "rigurd@tempest.com", "Rigurd Goblin Prime", "0901000015",
                    "Rigurd Stables", "City Center, Tempest",
                    "The oldest established ranch in Tempest", "gobta", "gobta@tempest.com",
                    "Gobta Rider", "0901000016", "Gobta Stables", "Outskirts, Tempest City",
                    "Promising young horse stables", "kaijin", "kaijin@tempest.com",
                    "Kaijin Craftsman", "0901000017", "Kaijin Stables",
                    "Craftsman District, Tempest", "Ranch combining modern tech and craftsmanship"};
            for (int i = 0; i < owners.length; i += 7) {
                if (!userRepository.existsByEmail(owners[i + 1])) {
                    jdbcTemplate.update(
                            ...
                            "9704123456" + owners[i + 3].substring(6), owners[i + 2].toUpperCase());
                }
                Integer uId = jdbcTemplate.queryForObject("SELECT id FROM users WHERE email = ?",
                        Integer.class, owners[i + 1]);
                if (uId != null) {
                    Integer profileCount = jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM horse_owner_profiles WHERE user_id = ?",
                            Integer.class, uId);
                    if (profileCount == null || profileCount == 0) {
                        jdbcTemplate.update(
                                ...
                                "038090" + owners[i + 3], "9704123456" + owners[i + 3].substring(6),
                                owners[i + 6]);
                    }
                }
            }
            ...
            for (Object[] jck : jockeys) {
                ...
                if (!userRepository.existsByEmail(email)) {
                    jdbcTemplate.update(
                            ...
                            jck[2].toString().toUpperCase());
                }
                Integer uId = jdbcTemplate.queryForObject("SELECT id FROM users WHERE email = ?",
                        Integer.class, email);
                if (uId != null) {
                    Integer profileCount = jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM jockey_profiles WHERE user_id = ?", Integer.class,
                            uId);
                    if (profileCount == null || profileCount == 0) {
                        jdbcTemplate.update(
            ...
            for (int i = 0; i < referees.length; i += 4) {
                if (!userRepository.existsByEmail(referees[i + 1])) {
                    jdbcTemplate.update(
                            ...
                            referees[i + 3], "97049999" + referees[i + 3].substring(6),
                            referees[i + 2].toUpperCase());
                } else {
                    jdbcTemplate.update(
                            "UPDATE users SET bank_name = 'ACB', bank_bin = '970416', bank_account_number = ?, bank_account_holder_name = ? WHERE email = ?",
                            "97049999" + referees[i + 3].substring(6),
                            referees[i + 2].toUpperCase(), referees[i + 1]);
                }
            }
            ...
            for (int i = 0; i < blacklisted.length; i += 5) {
                ...
                if (uId != null) {
                    Integer blCount = jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM blacklist WHERE target_id = ?", Integer.class,
                            uId);
                    if (blCount == null || blCount == 0) {
                        ...
                        jdbcTemplate.update("UPDATE blacklist SET reason = ? WHERE target_id = ?",
                                blacklisted[i + 4], uId);
                    }
                }
            }
    ...
    private void seedHorsesIfMissing() {
        try {
            ...
            String[][][] ownerHorseTemplates = {
                    // benimaru (Owner 1)
                    {{"Gojo Satoru", "4", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "98.5", "95",
                            "92", "Bay"},
                            {"Limitless Void", "5", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "96.0", "92", "90", "Black"},
                            {"Hollow Purple", "3", "FEMALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "94.2", "89", "88", "Chestnut"},
                            {"Six Eyes", "6", "MALE", "COMPLETED", "GOOD", "ACTIVE", "92.0", "90",
                                    "86", "White"},
                            {"Infinity Shield", "4", "FEMALE", "COMPLETED", "GOOD", "ACTIVE",
                                    "89.5", "86", "84", "Gray"},
                            {"Blue Sphere", "3", "MALE", "NOT_STARTED", "FAIR", "RESTING", "87.0",
                                    "84", "82", "Palomino"},
                            {"Red Attraction", "5", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "85.5",
                                    "82", "80", "Roan"},
                            {"Unbound Void", "4", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE", "83.0",
                                    "80", "78", "Brown"},
                            {"Falling Snow", "7", "FEMALE", "COMPLETED", "INJURED", "INJURED",
                                    "79.5", "76", "74", "White"},
                            {"Limitless Flash", "3", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "91.0", "88", "85", "Dark Bay"}},
                    // souei (Owner 2)
                    {{"Ryomen Sukuna", "5", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "99.0",
                            "96", "94", "Dark Chestnut"},
                            {"Malevolent Shrine", "6", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "97.5", "94", "91", "Black"},
                            {"Dismantle", "4", "FEMALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "93.0", "88", "87", "Bay"},
                            {"Cleave", "3", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "91.5", "87",
                                    "89", "Gray"},
                            {"Divine Flame", "5", "MALE", "COMPLETED", "GOOD", "ACTIVE", "90.0",
                                    "85", "83", "Chestnut"},
                            {"King of Curses", "6", "MALE", "COMPLETED", "GOOD", "ACTIVE", "88.5",
                                    "83", "81", "Roan"},
                            {"Furnace Open", "4", "FEMALE", "NOT_STARTED", "FAIR", "RESTING",
                                    "86.0", "81", "79", "Palomino"},
                            {"Spiderweb Slash", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE",
                                    "84.0", "79", "77", "Brown"},
                            {"Binding Vow", "7", "MALE", "COMPLETED", "INJURED", "INJURED", "81.0",
                                    "78", "75", "Black"},
                            {"Slicing Demon", "4", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "92.5", "89", "86", "Dun"}},
                    // hakuro (Owner 3)
                    {{"Yuji Itadori", "3", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE", "93.5",
                            "94", "88", "Chestnut"},
                            {"Black Flash", "4", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "95.0",
                                    "91", "90", "Black"},
                            {"Divergent Fist", "3", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "88.0",
                                    "86", "84", "Bay"},
                            {"Tiger of West", "5", "FEMALE", "COMPLETED", "GOOD", "ACTIVE", "86.5",
                                    "85", "82", "Palomino"},
                            {"Soul Striker", "4", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE", "84.5",
                                    "83", "80", "Gray"},
                            {"Vessel Spirit", "6", "MALE", "COMPLETED", "FAIR", "RESTING", "82.0",
                                    "81", "78", "Brown"},
                            {"Spatial Impact", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE",
                                    "80.5", "79", "76", "Roan"},
                            {"Unbreakable Will", "4", "MALE", "NOT_STARTED", "FAIR", "ACTIVE",
                                    "78.0", "77", "74", "Dark Bay"},
                            {"Iron Fortress", "7", "MALE", "COMPLETED", "INJURED", "INJURED",
                                    "76.5", "74", "71", "White"},
                            {"Cursed Spark", "3", "FEMALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "90.0", "88", "86", "Chestnut"}},
                    // geld (Owner 4)
                    {{"Megumi Fushiguro", "4", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "94.0",
                            "90", "91", "Black"},
                            {"Divine Mahoraga", "6", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "98.0", "97", "93", "White"},
                            {"Ten Shadows", "5", "MALE", "COMPLETED", "GOOD", "ACTIVE", "92.0",
                                    "89", "87", "Dark Bay"},
                            {"Chimera Garden", "4", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE",
                                    "89.0", "87", "85", "Gray"},
                            {"Divine Dog", "3", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "87.5", "84", "83", "Black"},
                            {"Nue Lightning", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE",
                                    "85.0", "82", "81", "Palomino"},
                            {"Great Serpent", "5", "MALE", "COMPLETED", "FAIR", "RESTING", "83.5",
                                    "80", "79", "Roan"},
                            {"Max Elephant", "6", "MALE", "COMPLETED", "GOOD", "ACTIVE", "81.5",
                                    "86", "76", "Gray"},
                            {"Rabbit Escape", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE",
                                    "79.0", "75", "77", "White"},
                            {"Shadow Step", "4", "FEMALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "88.5", "86", "84", "Chestnut"}},
                    // gabil (Owner 5)
                    {{"Nobara Kugisaki", "3", "FEMALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                            "91.0", "87", "89", "Chestnut"},
                            {"Straw Doll", "4", "FEMALE", "COMPLETED", "GOOD", "ACTIVE", "89.5",
                                    "85", "86", "Bay"},
                            {"Resonance", "5", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "93.0",
                                    "89", "88", "Black"},
                            {"Hairpin Strike", "3", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE",
                                    "87.0", "83", "84", "Roan"},
                            {"Nail Barrage", "4", "MALE", "IN_PROGRESS", "FAIR", "RESTING", "85.5",
                                    "81", "82", "Gray"},
                            {"Steel Hammer", "6", "MALE", "COMPLETED", "GOOD", "ACTIVE", "83.0",
                                    "80", "78", "Dark Bay"},
                            {"Voodoo Charm", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE", "81.0",
                                    "78", "76", "Palomino"},
                            {"Curse Needle", "5", "FEMALE", "COMPLETED", "INJURED", "INJURED",
                                    "79.5", "76", "75", "Brown"},
                            {"Iron Rose", "4", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE", "84.0",
                                    "82", "80", "Chestnut"},
                            {"Soul Piercer", "3", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "88.0", "86", "85", "Black"}},
                    // rigurd (Owner 6)
                    {{"Yuta Okkotsu", "4", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "97.0", "95",
                            "92", "Dark Bay"},
                            {"Queen of Curses", "5", "FEMALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "96.0", "93", "91", "Black"},
                            {"Rika Orimoto", "4", "FEMALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "94.5", "91", "89", "White"},
                            {"Pure Love", "3", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "92.5",
                                    "88", "87", "Chestnut"},
                            {"Copy Technique", "5", "MALE", "COMPLETED", "GOOD", "ACTIVE", "90.5",
                                    "87", "85", "Bay"},
                            {"Mutual Love", "6", "FEMALE", "COMPLETED", "FAIR", "RESTING", "88.0",
                                    "85", "83", "Gray"},
                            {"Ring of Vow", "3", "FEMALE", "NOT_STARTED", "GOOD", "ACTIVE", "86.0",
                                    "83", "81", "Palomino"},
                            {"Katana Slash", "4", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "84.0",
                                    "81", "79", "Roan"},
                            {"Special Grade", "7", "MALE", "COMPLETED", "INJURED", "INJURED",
                                    "82.5", "80", "77", "Dark Chestnut"},
                            {"Boundless Energy", "3", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE",
                                    "93.5", "92", "88", "Black"}},
                    // gobta (Owner 7)
                    {{"Toji Fushiguro", "6", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "98.0",
                            "94", "95", "Black"},
                            {"Sorcerer Killer", "7", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "96.5", "92", "93", "Dark Bay"},
                            {"Heavenly Restriction", "5", "MALE", "COMPLETED", "EXCELLENT",
                                    "ACTIVE", "95.0", "93", "91", "Gray"},
                            {"Inverted Spear", "4", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "93.0",
                                    "89", "88", "Bay"},
                            {"Playful Cloud", "5", "FEMALE", "COMPLETED", "GOOD", "ACTIVE", "91.0",
                                    "87", "86", "Chestnut"},
                            {"Split Soul Katana", "4", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE",
                                    "89.0", "85", "84", "Palomino"},
                            {"Chain of Thousand", "3", "MALE", "NOT_STARTED", "FAIR", "RESTING",
                                    "87.0", "83", "82", "Roan"},
                            {"Inventory Curse", "6", "MALE", "COMPLETED", "GOOD", "ACTIVE", "85.0",
                                    "81", "80", "Black"},
                            {"Apex Predator", "3", "MALE", "NOT_STARTED", "GOOD", "ACTIVE", "83.5",
                                    "79", "78", "Brown"},
                            {"Zero Cursed Power", "4", "FEMALE", "IN_PROGRESS", "EXCELLENT",
                                    "ACTIVE", "90.0", "88", "87", "Dark Chestnut"}},
                    // kaijin (Owner 8)
                    {{"Kento Nanami", "5", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "92.5", "91",
                            "90", "Chestnut"},
                            {"Ratio Technique", "4", "MALE", "COMPLETED", "GOOD", "ACTIVE", "90.5",
                                    "88", "88", "Bay"},
                            {"Seven Three", "3", "MALE", "IN_PROGRESS", "GOOD", "ACTIVE", "88.5",
                                    "86", "86", "Dark Bay"},
                            {"Overtime Unleashed", "6", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "94.0", "93", "89", "Gray"},
                            {"Blunt Blade", "4", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE", "86.5",
                                    "84", "83", "Black"},
                            {"Suguru Geto", "6", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE", "95.5",
                                    "92", "91", "Dark Chestnut"},
                            {"Cursed Manipulation", "5", "MALE", "COMPLETED", "EXCELLENT", "ACTIVE",
                                    "93.5", "90", "89", "Black"},
                            {"Uzumaki Spiral", "4", "FEMALE", "IN_PROGRESS", "GOOD", "ACTIVE",
                                    "91.5", "88", "87", "Gray"},
                            {"Night Parade", "3", "FEMALE", "NOT_STARTED", "FAIR", "RESTING",
                                    "87.5", "85", "83", "Palomino"},
                            {"Aoi Todo", "4", "MALE", "IN_PROGRESS", "EXCELLENT", "ACTIVE", "89.5",
                                    "90", "85", "Bay"}}};
            ...
            for (int oIdx = 0; oIdx < ownerUsernames.length; oIdx++) {
                ...
                if (horseCount != null && horseCount >= 10)
                    continue;

                // Clear existing horses for clean re-seeding if incomplete or old dataset
                if (horseCount != null && horseCount > 0) {
                    jdbcTemplate.update(
                            "DELETE FROM bets WHERE participant_id IN (SELECT id FROM race_participants WHERE horse_id IN (SELECT id FROM horses WHERE owner_id = ?))",
                            ownerId);
                    jdbcTemplate.update(
                            "DELETE FROM race_participants WHERE horse_id IN (SELECT id FROM horses WHERE owner_id = ?)",
                            ownerId);
    ...
    private void ensureUserConnections() {
        try {
            ...
            for (String[] conn : connections) {
                Integer rId = jdbcTemplate.queryForObject("SELECT id FROM users WHERE username = ?",
                        Integer.class, conn[0]);
                Integer cId = jdbcTemplate.queryForObject("SELECT id FROM users WHERE username = ?",
                        Integer.class, conn[1]);
                if (rId != null && cId != null) {
                    Integer count = jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM user_connections WHERE requester_id = ? AND recipient_id = ?",
                            Integer.class, rId, cId);
                    if (count == null || count == 0) {
                        jdbcTemplate.update(
                                "INSERT INTO user_connections (requester_id, recipient_id, status, created_at) VALUES (?, ?, 'ACCEPTED', GETDATE())",
                                rId, cId);
                    }
                }
            }
    ...
    private void ensureTournamentsAndRaces() {
        try {
            ...
            jdbcTemplate.update(
                    "UPDATE race_tracks SET name = 'Tempest Straight Track', location = 'Tempest City' WHERE id = 1 OR location LIKE '%SÃ¢n%' OR location LIKE '%Sân%'");
            jdbcTemplate.update(
                    "UPDATE race_tracks SET name = 'Jura Circular Track', location = 'Jura Great Forest' WHERE id = 2 OR location LIKE '%SÃ¢n%' OR location LIKE '%Sân%'");

            // Sanitize and update existing tournament names, locations, and descriptions to English
            jdbcTemplate.update(
                    "UPDATE tournaments SET tournament_name = 'Tempest Royal Tournament', location = 'Tempest Racetrack', description = 'Official speed tournament on the Tempest straight track - Completed Summer 2026' WHERE id = 1 OR tournament_name LIKE '%Tempest%' OR tournament_name LIKE '%Ho%ng Gia%' OR tournament_name LIKE '%HoÃ%'");
            jdbcTemplate.update(
                    "UPDATE tournaments SET tournament_name = 'Jura Championship Grand Prix', location = 'Jura Forest Racetrack', description = 'High-level circular track tournament in the Jura Forest - Completed Season 1' WHERE id = 2 OR tournament_name LIKE '%Jura%' OR tournament_name LIKE '%Ma Qu%c%' OR tournament_name LIKE '%CÃ°p%'");
            jdbcTemplate.update(
                    "UPDATE tournaments SET tournament_name = 'Rimuru Autumn Derby', location = 'Tempest City', description = 'Premier autumn speed race open for all qualified stables' WHERE id = 3 OR tournament_name LIKE '%Rimuru%'");
            jdbcTemplate.update(
                    "UPDATE tournaments SET tournament_name = 'Ingrassia Winter Cup', location = 'Ingrassia Racetrack', description = 'Grand winter tournament on synthetic turf' WHERE id = 4 OR tournament_name LIKE '%Ingrassia%'");
            jdbcTemplate.update(
                    "UPDATE tournaments SET location = 'Tempest Racetrack' WHERE location LIKE '%SÃ¢n%' OR location LIKE '%Sân%' OR location LIKE '%ThÃ³ng%' OR location LIKE '%Thắng%'");

            // Sanitize and update existing blacklist reasons to English
            jdbcTemplate.update(
                    "UPDATE blacklist SET reason = 'Violation of terms of service and fraudulent activity at Tempest' WHERE target_id = (SELECT id FROM users WHERE username = 'clayman') OR reason LIKE '%Vi ph%' OR reason LIKE '%Thao t%' OR reason LIKE '%mojibake%' OR reason LIKE '%c% c%c%'");
            jdbcTemplate.update(
                    "UPDATE blacklist SET reason = 'Disrupting racetrack order and intentional misconduct' WHERE target_id = (SELECT id FROM users WHERE username = 'footman') OR reason LIKE '%Qu%y r%i%'");
            jdbcTemplate.update(
                    "UPDATE blacklist SET reason = 'Illegal betting manipulation' WHERE target_id = (SELECT id FROM users WHERE username = 'laplace')");

            jdbcTemplate.update(
                    "UPDATE races SET race_name = 'Tempest Royal Sprint 1000m' WHERE id = 1 OR race_name LIKE '%Tempest%'");
            jdbcTemplate.update(
                    "UPDATE races SET race_name = 'Jura Championship 1600m' WHERE id = 2 OR race_name LIKE '%Jura%'");

            // Synchronize child race statuses for any active/open tournaments
            jdbcTemplate.update(
                    "UPDATE races SET status = 'OPEN_FOR_REGISTER' WHERE tournament_id IN (SELECT id FROM tournaments WHERE tournament_status IN ('ACTIVE', 'Active', 'OPEN_FOR_REGISTER')) AND status NOT IN ('FINISHED', 'CANCELLED')");

            Integer tCount =
                    jdbcTemplate.queryForObject("SELECT COUNT(*) FROM tournaments", Integer.class);
            if (tCount != null && tCount >= 4) {
                log.info(
                        "Tournaments already present (count={}) and updated to English. Skipping full seed.",
                        tCount);
                return;
            }

            Integer guyId = jdbcTemplate.queryForObject(
                    "SELECT id FROM users WHERE username = 'guy_crimson'", Integer.class);
            Integer velgryndId = jdbcTemplate.queryForObject(
                    "SELECT id FROM users WHERE username = 'velgrynd'", Integer.class);
            Integer luminousId = jdbcTemplate.queryForObject(
                    "SELECT id FROM users WHERE username = 'luminous'", Integer.class);
            Integer leonId = jdbcTemplate.queryForObject(
                    "SELECT id FROM users WHERE username = 'leon_cromwell'", Integer.class);

            List<Integer> trackIds =
                    jdbcTemplate.queryForList("SELECT id FROM race_tracks", Integer.class);
            Integer straightTrackId =
                    (trackIds != null && !trackIds.isEmpty()) ? trackIds.get(0) : 1;
            Integer ovalTrackId =
                    (trackIds != null && trackIds.size() > 1) ? trackIds.get(1) : straightTrackId;
            ...
            jdbcTemplate.update(
                    ...
                    guyId);
            Integer t1Id = jdbcTemplate.queryForObject(
                    "SELECT id FROM tournaments WHERE tournament_name = 'Tempest Royal Tournament'",
                    Integer.class);
            ...
            jdbcTemplate.update(
                    ...
                    velgryndId);
            Integer t2Id = jdbcTemplate.queryForObject(
                    "SELECT id FROM tournaments WHERE tournament_name = 'Jura Championship Grand Prix'",
                    Integer.class);
    ...
    private void seedCompletedRacesAndBets(Integer t1Id, Integer t1TrackId, Integer t1RefId,
            Integer t2Id, Integer t2TrackId, Integer t2RefId) {
        try {
            List<Integer> hIds = jdbcTemplate.queryForList("SELECT id FROM horses", Integer.class);
            List<Integer> jIds =
                    jdbcTemplate.queryForList("SELECT id FROM jockey_profiles", Integer.class);
            if (hIds == null || hIds.size() < 4 || jIds == null || jIds.size() < 4)
                return;
            ...
            Integer race1Id = jdbcTemplate.queryForObject(
                    "SELECT id FROM races WHERE race_name = 'Tempest Royal Sprint 1000m'",
                    Integer.class);

            // Participants for Race 1
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 1, 1, 62500, 16.0, 'FINISHED', GETDATE())",
                    race1Id, hVeldora, jRanga);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 2, 2, 63800, 15.67, 'FINISHED', GETDATE())",
                    race1Id, hShadow, jBeretta);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 3, 3, 65100, 15.36, 'FINISHED', GETDATE())",
                    race1Id, hSword, jDiablo);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 4, 4, 67200, 14.88, 'FINISHED', GETDATE())",
                    race1Id, hBold, jCarrion);

            Integer p1Win = jdbcTemplate.queryForObject(
                    "SELECT id FROM race_participants WHERE race_id = ? AND final_rank = 1",
                    Integer.class, race1Id);
            Integer p2Loss = jdbcTemplate.queryForObject(
                    "SELECT id FROM race_participants WHERE race_id = ? AND final_rank = 2",
                    Integer.class, race1Id);

            // Spectators placing bets on Race 1
            Integer shunaId = jdbcTemplate
                    .queryForObject("SELECT id FROM users WHERE username='shuna'", Integer.class);
            Integer shionId = jdbcTemplate
                    .queryForObject("SELECT id FROM users WHERE username='shion'", Integer.class);
            Integer milimId = jdbcTemplate
                    .queryForObject("SELECT id FROM users WHERE username='milim'", Integer.class);
            ...
            Integer race2Id = jdbcTemplate.queryForObject(
                    "SELECT id FROM races WHERE race_name = 'Jura Championship 1600m'",
                    Integer.class);

            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 1, 1, 98200, 16.29, 'FINISHED', GETDATE())",
                    race2Id, hSword, jDiablo);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 2, 2, 99500, 16.08, 'FINISHED', GETDATE())",
                    race2Id, hVeldora, jRanga);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 3, 3, 101200, 15.81, 'FINISHED', GETDATE())",
                    race2Id, hShadow, jBeretta);
            jdbcTemplate.update(
                    "INSERT INTO race_participants (race_id, horse_id, jockey_id, gate_number, final_rank, finish_time, average_speed, status, created_at) VALUES (?, ?, ?, 4, 4, 103400, 15.47, 'FINISHED', GETDATE())",
                    race2Id, hBold, jCarrion);
    ...
    private void ensureRichWalletTransactions() {
        try {
            Integer txCount = jdbcTemplate
                    .queryForObject("SELECT COUNT(*) FROM wallet_transactions", Integer.class);
            ...
            String[] usernames =
                    {"shuna", "shion", "milim", "benimaru", "souei", "guy_crimson", "ranga"};
            for (String uname : usernames) {
                ...
                Integer wId = jdbcTemplate.queryForObject(
                        "SELECT id FROM wallets WHERE user_id = ?", Integer.class, uId);
                if (wId == null)
                    continue;
