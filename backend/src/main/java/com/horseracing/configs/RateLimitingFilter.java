 ...
 * Uses IP-based rate limiting with Bucket4j token bucket algorithm and Caffeine Cache
 * bounded eviction (10-minute access TTL, max 10,000 IPs).
 *
 * Limits:
...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        switch (path) {
            case "/api/auth/login", "/api/auth/google" ->
                bucket = loginBuckets.get(clientIp, k -> createBucket(10, 1));
            case "/api/auth/verify-reset-otp", "/api/auth/reset-password" ->
                bucket = otpBuckets.get(clientIp, k -> createBucket(5, 1));
            case "/api/auth/forgot-password" ->
                bucket = forgotPasswordBuckets.get(clientIp, k -> createBucket(3, 1));
            default -> {}
        }
    ...
    private String getClientIp(HttpServletRequest request) {
        ...
        // Only parse X-Forwarded-For if connection originates from local loopback or private network proxy
