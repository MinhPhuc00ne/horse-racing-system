...
public class RaceServiceTest {
    ...
    void testCreateRace_MaxHorsesInvalid() {
        CreateRaceRequest request = CreateRaceRequest.builder()
                .raceName("Race 1")
                .tournamentId(1)
                .raceTrackId(1)
                ...
                .raceRound(1)
                .maxHorses(5) // Invalid, must be 7, 8, 12
                .distance(1200.0)
                .build();

        Exception exception = assertThrows(RuntimeException.class, () -> raceService.createRace(request));
        assertEquals("Maximum participating horses must be either 7, 8, or 12", exception.getMessage());
    }
    ...
    void testCreateRace_TimeOverlaps() {
        ...
        Exception exception = assertThrows(RuntimeException.class, () -> raceService.createRace(request));
        assertEquals("Race timing overlaps with another race on the same track", exception.getMessage());
    }
    ...
    void testSubmitRegistration_SharePercentInvalid() {
        RegisterRaceRequest request = RegisterRaceRequest.builder()
                ...
                .jockeySharePercent(20.0) // 70 + 20 = 90% (invalid)
                .build();
    ...
    void testApproveRegistration_GateAssignment() {
        ...
        verify(raceParticipantRepository, times(1)).save(argThat(p -> p.getGateNumber() == 3)); // gate number should be count + 1 = 3
    }
}
