...
public class PredictionPayoutServiceTest {
    ...
    @BeforeEach
    public void setUp() {
        spectator1 = User.builder().id(11).fullName("Spectator One").email("spec1@test.com")
                .role(Role.SPECTATOR).build();

        spectator2 = User.builder().id(12).fullName("Spectator Two").email("spec2@test.com")
                .role(Role.SPECTATOR).build();

        admin = User.builder().id(1).fullName("System Admin").email("admin@test.com")
                .role(Role.ADMIN).build();

        race = Race.builder().id(101).raceName("Golden Cup Race").build();

        participant1 = RaceParticipant.builder().id(201).race(race)
                .horse(Horse.builder().id(301).name("Fast Wind").build()).finalRank(1).build();

        participant2 = RaceParticipant.builder().id(202).race(race)
                .horse(Horse.builder().id(302).name("Thunder").build()).finalRank(2).build();

        participant3 = RaceParticipant.builder().id(203).race(race)
                .horse(Horse.builder().id(303).name("Silver").build()).finalRank(3).build();
    }

    @Test
    public void testProcessPayouts_WinBetSuccess() {
        Bet bet1 = Bet.builder().id(1001).user(spectator1).race(race).participant(participant1) // WIN,
                                                                                                // rank
                                                                                                // 1
                .amount(BigDecimal.valueOf(100.0)).betType("WIN").status("PENDING").build();

        Bet bet2 = Bet.builder().id(1002).user(spectator2).race(race).participant(participant2) // WIN,
                                                                                                // rank
                                                                                                // 2
                                                                                                // (LOST)
                .amount(BigDecimal.valueOf(200.0)).betType("WIN").status("PENDING").build();
        ...
        when(walletRepository.findByUserIdWithLock(1)).thenReturn(
                Optional.of(Wallet.builder().id(99).user(admin).balance(BigDecimal.ZERO).build()));
        ...
        // Total WIN pool = 300. Net WIN pool (90%) = 270. Only bet1 won on participant1. Odds = 270
        // / 100 = 2.70
        ...
        verify(notificationService, times(2)).sendNotification(any(User.class), anyString(),
                anyString(), any(NotificationType.class));
    }

    @Test
    public void testProcessPayouts_PlaceBetSuccess() {
        Bet bet1 = Bet.builder().id(1001).user(spectator1).race(race).participant(participant1) // PLACE
                                                                                                // on
                                                                                                // Rank
                                                                                                // 1
                                                                                                // (WON)
                .amount(BigDecimal.valueOf(100.0)).betType("PLACE").status("PENDING").build();

        Bet bet2 = Bet.builder().id(1002).user(spectator2).race(race).participant(participant2) // PLACE
                                                                                                // on
                                                                                                // Rank
                                                                                                // 2
                                                                                                // (WON)
        ...
        lenient().when(walletRepository.findByUserIdWithLock(1)).thenReturn(
                Optional.of(Wallet.builder().id(99).user(admin).balance(BigDecimal.ZERO).build()));
    ...
    public void testRefundBetsForParticipant() {
        Bet bet = Bet.builder().id(1005).user(spectator1).race(race).participant(participant1)
                .amount(BigDecimal.valueOf(150.0)).status("PENDING").build();

        Wallet wallet =
                Wallet.builder().id(501).user(spectator1).balance(BigDecimal.valueOf(20.0)).build();
        ...
        predictionPayoutService.refundBetsForParticipant(participant1, "REJECTED",
                "Horse disqualified. Bet refunded {amount} VND.");
        ...
        verify(notificationService, times(1)).sendNotification(eq(spectator1), anyString(),
                eq("Horse disqualified. Bet refunded 150.0 VND."), eq(NotificationType.WALLET));
    }

    @Test
    public void testRefundBetsForRace() {
        Bet bet1 = Bet.builder().id(1006).user(spectator1).race(race)
                .amount(BigDecimal.valueOf(50.0)).status("PENDING").build();

        Bet bet2 = Bet.builder().id(1007).user(spectator2).race(race)
                .amount(BigDecimal.valueOf(100.0)).status("PENDING").build();
        ...
        verify(notificationService, times(2)).sendNotification(any(User.class), anyString(),
                anyString(), eq(NotificationType.WALLET));
    }
}
