# Hướng dẫn Tích hợp API: Admin Tạo Giải đấu & Vòng đua (1 Giải đấu = 1 Vòng đua)

Tài liệu này tổng hợp toàn bộ các API đã hoàn thiện phục vụ cho luồng **Admin tạo giải đấu** (Vòng đua chính của giải đấu được tự động khởi tạo ngầm, không cần tạo thủ công). 
...
### 8. Tạo Vòng Đua thủ công (Manual Race Creation - Bị giới hạn)
* **API Endpoint**: `POST /api/admin/races`
* **Mô tả**: Gửi yêu cầu tạo thêm vòng đua cho giải đấu. 
* **Lưu ý đặc biệt**: Mỗi giải đấu chỉ được phép có đúng **một Vòng đua duy nhất** và hệ thống đã **tự động tạo vòng đua mặc định** ngay khi tạo giải đấu (`POST /api/admin/tournaments`). Do đó, nếu gọi lại API này cho cùng một giải đấu, hệ thống sẽ trả về lỗi `400 Bad Request` với thông báo: `"This tournament already has a race. Only 1 race is allowed per tournament."`
* **Headers**:
  * `Authorization: Bearer <access_token_cua_admin>`
  * `Content-Type: application/json`
* **Request Body (JSON)**:
  ...
  ```
* **Response (JSON - Trường hợp lỗi do đã có vòng đua)**:
  ```json
  {
    "status": 400,
    "message": "This tournament already has a race. Only 1 race is allowed per tournament."
  }
  ```
