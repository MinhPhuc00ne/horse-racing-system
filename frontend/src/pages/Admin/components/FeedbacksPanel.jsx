        ...
        >
          ...
          >
            <h3 className="ho-font-epilogue fs-5 fw-bold mb-1" style={{ color: resolveModal.actionType === 'REJECT' ? '#ef4444' : 'var(--ho-primary-dark)', borderBottom: '1px solid rgba(0, 0, 0, 0.08)', paddingBottom: '10px', margin: 0 }}>
              {resolveModal.actionType === 'REJECT' ? `Reject Feedback #${resolveModal.feedbackId}` : `Resolve Feedback #${resolveModal.feedbackId}`}
            </h3>

            <div className="form-group text-start">
              <label className="ho-input-label">
                {resolveModal.actionType === 'REJECT' ? 'Rejection Reason *' : 'Resolution Notes / Response *'}
              </label>
              <textarea
                className="ho-form-input text-dark fw-semibold"
                rows="5"
                required
                disabled={resolving}
                placeholder={resolveModal.actionType === 'REJECT' ? 'Enter rejection reason to notify the user...' : 'Enter resolution steps or response to the user...'}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', borderTop: '1px solid rgba(0, 0, 0, 0.08)', paddingTop: '15px' }}>
              ...
              >
                {resolving ? 'Saving...' : (resolveModal.actionType === 'REJECT' ? 'Confirm Reject' : 'Confirm Resolve')}
              </button>
            </div>
          </form>
        </div>,
