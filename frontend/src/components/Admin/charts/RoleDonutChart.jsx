    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ width: '130px', height: '130px', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', pointerEvents: 'none' }}>
           <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#143422' }}>{totalUsers}</div>
           <div style={{ fontSize: '8px', color: '#64748b', letterSpacing: '0.05em', fontWeight: '700' }}>TOTAL USERS</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '8px', marginTop: '10px', fontSize: '9.5px' }}>
        {data.map((entry) => (
          <div key={entry.name} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <span style={{ color: '#334155' }}>{entry.name.substring(0, 4)}: <strong style={{ color: '#143422' }}>{entry.value}</strong></span>
          </div>
        ))}
      </div>
    </div>
